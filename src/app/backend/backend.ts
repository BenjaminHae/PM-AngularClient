import { EncryptionWrapper } from './cryptoWrapper';
import { Account } from './account';
import * as CryptoJS from 'crypto-js';

function callPlugins(a?, b?) {
...
//mixin for authenticatedSession
let AuthenticatedSession = (superclass) => class extends superclass {
    ...
    untrustAndLogout() {
        ...
        localStorage.clear();
        var promises = [];
        var device = CookieHandler.getCookie('device');
        if ((device != null) && (device != "")) {
            promises.push(self.doPost("deletepin", {'user': CookieHandler.getCookie('username'), 'device': device}));
        }
        return Promise.all(promises)
            .then(function(){
                self.deleteCookie('device');
...
let PinHandling = (superclass) => class extends superclass {
    getDevice() {
        var self = this;
        var device:string = CookieHandler.getCookie('device');
        if (device != null) {
    ...
    delPin() {
        ...
        var device = CookieHandler.getCookie('device');
        if((device != null) && (device != "")) {
            return self.doPost("deletepin", {'user': self.getCookie('username'), 'device': device})
                .then(function(msg){
...
//Backend class
export class AccountBackend extends mix(commonBackend).with(EventHandler, AuthenticatedSession, Timeout, PinHandling, Accounts) {
    constructor() {
        super();
...
export class HistoryBackend extends mix(commonBackend).with(EventHandler, AuthenticatedSession, Timeout, PinHandling) {
    getHistory() {
        var self = this;
...
export class LogonBackend extends mix(commonBackend).with(EventHandler, PinHandling) {
    doPost(endpoint, data?) {
        data = data || {};
    ...
    doPinLogin(pin) {
        ...
        return self.doPost('getpinpk', {user:user, device:self.getCookie('device'), sig:sig})
            .then(function(msg) {
